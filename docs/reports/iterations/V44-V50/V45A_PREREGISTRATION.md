...
- `boundary_weight_ij` and `safe_homo_weight_ij` come from the same unified
  topology contraction used for all datasets.
- The masks are detached for the first implementation so the frequency loss
  cannot create an artificial edge gap by moving topology assignments.
...
The training reference must be a frozen warmup reference from the same run, not
a historical baseline table and not a dataset-specific ceiling:

```text
warmup_epochs = W
v45a losses are inactive during epochs 1..W
band_reference = stopgrad(mean_or_ema_{epochs 1..W}(band_mass))
band_reference = stopgrad(band_reference - band_reference_delta)
```

Preregistered first implementation constants:
...
The per-dataset band ceilings used in first-stage gates may not be used inside
the training objective or `safe_band_gate`.
...
The reference is graph-adaptive because it is measured from the same graph and
the same unified forward path before v45a losses become active; it is not
dataset-routed because it does not read dataset identity, labels, or graph type.
...
During warmup epochs, v45a edge-local frequency and band-guard losses must be
inactive. Diagnostics may be logged during warmup, but they must not affect
training until the frozen reference is finalized.
...
v44b_enabled=false
v45a_enabled=true
v45a_band_reference_ready=true after warmup
...
```text
v45a_band_reference_ready = true
v45a_warmup_epoch_count = 5
...
These ceilings are evaluation gates only. They must not be used as
`band_reference` during training.
...
- `v45a_band_reference_ready=false` after warmup
