...
from utils import random_node_pairs, sparse_spmm


def subspace_smoothness_loss(u: torch.Tensor, norm_adj: torch.Tensor) -> torch.Tensor:
    """ELSS-style manifold smoothness on a sparse normalized graph."""
    diffused = sparse_spmm(norm_adj, u)
    lap_u = u - diffused
    energy = torch.sum(u * lap_u)
